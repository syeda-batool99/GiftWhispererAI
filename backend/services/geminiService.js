...
async function getGiftRecommendations(quizAnswers) {
  ...
  const prompt = `
    ...
    Based on the following personality assessment and gift context, please provide 5 unique and creative gift recommendations in a JSON array format. Each object in the array should have "name", "description", "reason", and "image_prompt" properties. The "image_prompt" should be a short, visually descriptive prompt for an image generation model.
  ...
  try {
    ...
    try {
      ...
      const giftsWithImages = await Promise.all(gifts.map(async (gift) => {
        const imageUrl = await generateImage(gift.image_prompt);
